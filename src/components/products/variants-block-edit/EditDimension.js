...
const EditDimension = ({editVariant, colorIncluded, variantData}) => {
...
const WithColor = ({editVariant, variantData}) => {
    ...
    const [options,setOptions]=useState(variantData.item.colors)
    const [colors,setColors]=useState(COLORS)
    const [values, setValues] = useState({
      color: variantData.item.colors,
      quantityStock: variantData.item.quantityStock,
      variantValue:variantData.item.dimensions
    });
    ...
    const addVariant=()=>{
      ...
      const data = {
        ...
        dimensions: values.variantValue,
        colors: colorArr
      }
    ...
    <div className='card flex flex-column w-full mt-4 surface-200'>
      ...
      <div className='w-full flex justify-content-between'>
        ...
        <InputText
          ...
          <label className='text-sm'>dimension ex: 16x16</label>
        </div>
